    ...
    fun migrateFrom324To325(db: SQLiteDatabase)
    {
        db.transaction {
            try
            {
                ...
                // add creation_date to missing tables
                db.execSQL("ALTER TABLE $TABLE_FIELD_DATA_OPTION ADD COLUMN $COLUMN_CREATION_DATE INTEGER DEFAULT 0")
                db.execSQL("ALTER TABLE $TABLE_FIELD_OPTION ADD COLUMN $COLUMN_CREATION_DATE INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE `$TABLE_FILTER` ADD COLUMN $COLUMN_CREATION_DATE INTEGER DEFAULT 0")
                db.execSQL("ALTER TABLE $TABLE_MAP_TILE_REGION ADD COLUMN $COLUMN_CREATION_DATE INTEGER DEFAULT 0")
                db.execSQL("ALTER TABLE $TABLE_RULE ADD COLUMN $COLUMN_CREATION_DATE INTEGER DEFAULT 0")
                db.execSQL("ALTER TABLE $TABLE_USER ADD COLUMN $COLUMN_CREATION_DATE INTEGER DEFAULT 0")

                // Config
                db.execSQL("ALTER TABLE $TABLE_CONFIG ADD COLUMN $COLUMN_CONFIG_IS_ARCHIVED INTEGER DEFAULT 0")
                db.execSQL("ALTER TABLE $TABLE_CONFIG ADD COLUMN $COLUMN_CONFIG_GEOFENCE_IS_ENABLED INTEGER DEFAULT 0")
                db.execSQL("ALTER TABLE $TABLE_CONFIG ADD COLUMN $COLUMN_CONFIG_GEOFENCE_BUFFER_VALUE INTEGER DEFAULT 5")
                db.execSQL("ALTER TABLE $TABLE_CONFIG ADD COLUMN $COLUMN_CONFIG_ALLOW_SUPERVISOR_EDITS INTEGER DEFAULT 1")

                // EnumerationItem - review/exclusion
                db.execSQL("ALTER TABLE $TABLE_ENUMERATION_ITEM ADD COLUMN $COLUMN_ENUMERATION_ITEM_REVIEW_STATUS TEXT DEFAULT 'Ignore'")
