...
class BarcodeScannerProcessor(context: Context) : VisionProcessorBase<List<Barcode>>(context) {
  ...
  private var barcodeLiveData: MutableLiveData<Barcode>? = null
  ...
  override fun onSuccess(barcodes: List<Barcode>, graphicOverlay: GraphicOverlay) {
    ...
    {
      barcodeLiveData!!.value = barcodes[0]
      stop()
    }
...
//    }
  ...
  fun barcodeProvider(): LiveData<Barcode?>? {
    if (barcodeLiveData == null) {
      barcodeLiveData = MutableLiveData<Barcode>()
    }
